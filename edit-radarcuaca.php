...
if (isset($_GET['tanggal'])) {
    $sqlEdit = mysqli_query($connect, "select * from radarcuaca where Tanggal='$_GET[tanggal]'");
    $i=0;
...
<head>
  <title>Form Pemeliharaan Radar Cuaca</title>
...
?><script>
    ...
    <div class="form-group"><label for="ti">Nama :</label><input type="text"
          class="form-control" id="np" name="np" value="<?=$nama?>" placeholder="Nama Petugas/Teknisi"></div>
      <div class="form-group"><label for="ti">Tanggal Pemeliharaan : <?=$tanggal?></label><input type="hidden"
          class="form-control input-tanggal" id="tp" name="tp" placeholder="Tanggal Perawatan" value="<?=$tanggal?>"></div>
      <table class="table">
        <thead>
          <tr>
            <th scope="col">No</th>
        ...
        <tbody>
        <?php
$namaperalatan = array('DEHYDRATOR', 'ANTENA & PEDESTAL', 'BLOK CABINET FAN', 'BLOK MAGNETRON', 'ANTENNA CONTROL UNIT', 'BLOK TRANSMITTER', 'BLOK RECEIVER', 'BLOK KABINET TRANSMITTER', 'OBSTRUCTION LIGHTS', 'UPS RADAR', 'Genset Radar', 'PC Server', 'CLIENT FCT 1', 'CLIENT FCT 2', 'CLIENT APP', 'CLIENT OBS');
$merksn = array('GEMATRONIK', 'GEMATRONIK', 'GEMATRONIK', 'GEMATRONIK', 'GEMATRONIK', 'GEMATRONIK', 'GEMATRONIK', 'GEMATRONIK', '-', 'APC', 'DEUTZ', 'DELL', 'DELL', 'LENOVO', 'LENOVO', 'LENOVO');
...
?>
      ...
      <div class="form-group"><label for="ti">Catatan Khusus : </label><textarea
          ...
          <input type="hidden" name="namaalat" value="radarcuaca">
