...
if(isset($_GET['tanggal'])){
    ...
    $query=mysqli_query($connect, "(SELECT * FROM alatpt WHERE Tanggal='$tanggal') ORDER by Tanggal ASC");
    $i=0;
...
//if($_POST['tp']=="") header("Location: pemeliharaan-alatpt.php");
if (isset($_POST['simpan']) && ($_POST['tp'] != "") && ($_POST['np'] != "")) {
    ...
    for ($i = 0; $i < $max; $i++) {
        ...
        $merk[$i] = $_POST["merk$i"];
        mysqli_query($connect, "INSERT INTO alatpt VALUES ('','$tanggal','$alat[$i]','$merk[$i]','$kondisi[$i]','$nama','$catatan')");
    }
}
...
$pdf->Cell(40,5, ': PERALATAN TEKNIS',0,0, 'L');
...
$pdf->Cell(30,5,'PC STAF PERTEK','LRT',0,'L');
...
$pdf->Cell(30,5,'CLOUD SHARING','LRT',0,'L');
$pdf->Cell(40,5,'SEDERHANA ELEKTRONIK',1,0,'C');
$pdf->Cell(10,5,'1 UNIT',1,0,'C');
if($kondisi[4]==0){
    $pdf->Cell(20,5,'X',1,0,'C');
...
$pdf->MultiCell(0,5,'');
$pdf->Cell(7,5,'2',1,0,'C');
$pdf->Cell(30,5,'HUB 5 PORT','LRT',0,'L');
$pdf->Cell(40,5,'SEDERHANA ELEKTRONIK',1,0,'C');
$pdf->Cell(10,5,'1 UNIT',1,0,'C');
if($kondisi[6]==0){
    $pdf->Cell(20,5,'X',1,0,'C');
