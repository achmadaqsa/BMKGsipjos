...
if(isset($_GET['tanggal'])){
    ...
    $query=mysqli_query($connect, "(SELECT * FROM `adjuanda` WHERE Tanggal='$tanggal') ORDER by Tanggal ASC");
    $i=0;
...
//if($_POST['tp']=="") header("Location: pemeliharaan-adjuanda.php");
if (isset($_POST['simpan']) && ($_POST['tp'] != "") && ($_POST['np'] != "")) {
    ...
    for ($i = 0; $i < $max; $i++) {
        ...
        mysqli_query($connect, "INSERT INTO adjuanda VALUES ('','$tanggal','$alat[$i]','$kondisi[$i]','$nama','$catatan')");
    }
}
...
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(20, 5, 'Teknisi', 1, 0, 'L');
$pdf->Cell(40, 5,$nama, 0, 1, 'L');
//-------------------------------------------------------------------------
//TABEL TERMINAL 1
$pdf->MultiCell(0, 5, '', 0, 2);
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(7, 5, 'A. Display Cuaca Bandara Terminal 1 Juanda Surabaya', 0, 2, 'L');
...
$pdf->MultiCell(0, 5, '', 0, 2);
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(80, 5, 'Cek pengkabelan (powee dan VGA)',1, 0, 'L');
if($kondisi[6]==0){
    $pdf->Cell(20, 5,'X',1,0, 'C');
}
if($kondisi[6]==1){
...
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(7, 5, 'B. Display Cuaca Bandara Terminal 2 Juanda Surabaya', 0, 2, 'L');
