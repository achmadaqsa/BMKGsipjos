...
if (isset($_GET['tanggal'])) {
    ...
    $query=mysqli_query($connect, "(SELECT * FROM alatda WHERE Tanggal='$tanggal') ORDER by Tanggal ASC");
    $i=0;
...
//if($_POST['tp']=="") header("Location: pemeliharaan-alatda.php");
if (isset($_POST['simpan']) && ($_POST['tp'] != "") && ($_POST['np'] != "")) {
    ...
    for ($i = 0; $i < $max; $i++) {
        ...
        $merk[$i] = $_POST["merk$i"];
        mysqli_query($connect, "INSERT INTO alatda VALUES ('','$tanggal','$alat[$i]','$merk[$i]','$kondisi[$i]','$nama','$catatan')");
    }
}
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(180, 5, 'Ground Receiver Radiosonde', 0, 1, 'C');
//----------------------------------------------------------------------------------
//identitas
...
$pdf->Cell(133, 5, ':Gedung Aerologi/Stasiun Meteorologi Kelas I Juanda Sidoarjo', 'LR', 1, 'L');
...
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 5, 'II. Kondisi Komponen Ground Receiver Radiosonde', 0, 1);
$pdf->SetFont('Arial', 'B', 8);
...
$pdf->Cell(40, 5, 'Antenna RF Receiver', 1, 0, 'L');
$pdf->Cell(55, 5, 'MEISEI/RD-06G-001/04891', 1, 0, 'C');
if ($kondisi[0] == 0) {
...
$pdf->Cell(40, 5, 'Antenna GPS', 1, 0, 'L');
$pdf->Cell(55, 5, 'SANAV/S-200/00005371', 1, 0, 'C');
if ($kondisi[1] == 0) {
    $pdf->Cell(9, 5, 'V', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
...
$pdf->Cell(40, 5, 'Reapeter Antenna GPS', 1, 0, 'L');
$pdf->Cell(55, 5, 'MEISEI/RD-06G-005/04911', 1, 0, 'C');
if ($kondisi[2] == 0) {
    $pdf->Cell(9, 5, 'V', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
...
$pdf->Cell(40, 5, 'Receiver', 1, 0, 'L');
$pdf->Cell(55, 5, 'MEISEI/-/53471', 1, 0, 'C');
if ($kondisi[3] == 0) {
    $pdf->Cell(9, 5, 'V', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
...
$pdf->Cell(40, 5, 'Sonde Checker', 1, 0, 'L');
$pdf->Cell(55, 5, 'MEISEI/-/53762', 1, 0, 'C');
if ($kondisi[4] == 0) {
    $pdf->Cell(9, 5, 'V', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
...
$pdf->Cell(40, 5, 'PC Pengolahan Radiosonde', 'LRT', 0, 'L');
$pdf->Cell(55, 10, 'DELL/OPTILEX 3046', 1, 0, 'C');
if ($kondisi[5] == 0) {
...
$pdf->SetFont('Arial', '', 7);
$pdf->Cell(7, 5, '', 0, 0, 'L');
$pdf->Cell(40, 5, 'dan Pibal', 'LRB', 0, 'L');
//----------------------------------------------------------------------
//catatan khusus
...
$pdf->Cell(180, 5, 'LAPORAN PEMELIHARAAN PERALATAN', 0, 1, 'C');
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(180, 5, 'Theodolite Pibal & Barometer Air Raksa', 0, 1, 'C');
//----------------------------------------------------------------------------------
//identitas
$pdf->Cell(0, 2, '', 0, 1);// Memberikan space kebawah agar tidak terlalu rapat
$pdf->SetFont('Arial', 'B', 10);
...
$pdf->Cell(0, 5, '', 0, 1);// Memberikan space kebawah agar tidak terlalu rapat
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 5, 'II. Kondisi Komponen Peralatan Theodolit Pibal dan Barometer Air Raksa', 0, 1);
$pdf->SetFont('Arial', 'B', 8);
$pdf->Cell(7, 10, 'No', 1, 0, 'C');
$pdf->Cell(40, 10, 'Nama Peralatan', 1, 0, 'C');
$pdf->Cell(55, 10, 'Merk / SN', 1, 0, 'C');
...
$pdf->SetFont('Arial', '', 8);
$pdf->Cell(7, 5, '1', 1, 0, 'C');
$pdf->Cell(40, 5, 'Theodolit Pibal', 1, 0, 'L');
$pdf->Cell(55, 5, 'BREITHAUPT KASSEL/-/463930', 1, 0, 'C');
if ($kondisi[6] == 0) {
    $pdf->Cell(9, 5, 'V', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
    $pdf->Cell(9, 5, '', 1, 0, 'C');
