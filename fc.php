...
if(isset($_GET['tanggal'])){
    ...
    $query=mysqli_query($connect, "(SELECT * FROM alatfc WHERE Tanggal='$tanggal') ORDER by Tanggal ASC");
    $i=0;
...
//if($_POST['tp']=="") header("Location: pemeliharaan-alatfc.php");
if (isset($_POST['simpan']) && ($_POST['tp'] != "") && ($_POST['np'] != "")) {
    ...
    for ($i = 0; $i < $max; $i++) {
        ...
        $merk[$i] = $_POST["merk$i"];
        mysqli_query($connect, "INSERT INTO alatfc VALUES ('','$tanggal','$alat[$i]','$merk[$i]','$kondisi[$i]','$nama','$catatan')");
    }
}
...
$pdf->Cell(40,5, ': FORECASTER',0,0, 'L');
...
$pdf->Cell(30,5,'PC KAPOK','LRT',0,'L');
...
$pdf->Cell(25,10,'R. FORECASTER',1,0,'C');
$pdf->Cell(0,10,'',1,0,'C');

$pdf->MultiCell(0,5,'');
$pdf->Cell(7,5,'',0,0,'C');
$pdf->Cell(30,5,'FORECASTER','LRB',0,'L');

 $pdf->MultiCell(0,5,'');
 $pdf->Cell(7,10,'2',1,0,'C');
 $pdf->Cell(30,5,'PC STAF','LRT',0,'L');
 $pdf->Cell(40,10,'SEDERHANA ELEKTRONIK',1,0,'C');
 $pdf->Cell(10,10,'1 UNIT',1,0,'C');
 if($kondisi[1]==0){
    $pdf->Cell(20,10,'X',1,0,'C');
...
$pdf->Cell(30,5,'HUB 16 PORT','LRT',0,'L');
...
$pdf->Cell(25,5,'R. FORECASTER',1,0,'C');
$pdf->Cell(0,5,'',1,0,'C');
...
if($kondisi[0]==1){
 ...
 $pdf->Cell(30,5,'PRINTER-SCANNER',1,0,'L');
 $pdf->Cell(40,5,'SEDERHANA ELEKTRONIK',1,0,'C');
 $pdf->Cell(10,5,'2 UNIT',1,0,'C');
 if($kondisi[0]==0){
    $pdf->Cell(20,5,'X',1,0,'C');
